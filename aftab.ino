...
unsigned long lastGetTime = 0;  // Assuming it's a timestamp
const unsigned long GET_INTERVAL = 60000;  // 60 seconds
//---------------------------------------------------------------------------------------------------------
// Enter Google Script Deployment ID:

const char *GScriptId = "AKfycbwkiLqc_v3ytxXsmfevoJFtjL-rDACY-CxAOehY612GYfKHizNn0whxQnVcTxqCez2f";
String gate_number = "1";
String cellRange = "Sheet1!K2";  // Replace "Sheet1" with the actual sheet name

//---------------------------------------------------------------------------------------------------------
// Enter network credentials:
const char* ssid     = "DESKTOP-JMTJ8LU 7086";
const char* password = "Utk25062001";
//---------------------------------------------------------------------------------------------------------
...
String url1 = String("/macros/s/") + GScriptId + "/exec?range=" + cellRange;
...
void loop() {
  ...
  //----------------------------------------------------------------
   ...
   if (digitalRead(IR) == LOW) {
    // Object detected without scanning a card
    Serial.println("Object detected without scanning a card");
    lcd.clear();
    lcd.setCursor(0,0);
  ...
  if(client->POST(url, host, payload)){ 
    ...
    // Make a GET request after displaying "Thanks"
    Serial.println("Making a GET request...");
    
    // Modify the url to include the cell range
    String getURL = url1+"&command=get_data"
;
    if (client->GET(getURL, host)) {
      // Process the response if needed
      Serial.println("[OK] GET request successful");

      // Extract and print the response data
      String response = client->getResponseBody();
      Serial.println("Response: " + response);
      int responseData = response.toInt();
  if (responseData == 0) {
    // Play the buzzer sound
  ...
  noTone(BUZZER);
      ...
      Serial.println("Error while making GET request");
    }
  }
  //----------------------------------------------------------------
  else {
